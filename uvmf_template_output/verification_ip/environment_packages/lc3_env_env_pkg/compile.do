# Tcl do file for compile of lc3_env interface
...
eval $cmd
# Include build for sub-environment control_env_pkg
quietly set cmd [format "source %s/environment_packages/control_env_pkg/compile.do" $env(UVMF_VIP_LIBRARY_HOME)]
eval $cmd
...
quietly set cmd [format "vlog -timescale 1ps/1ps +incdir+%s/environment_packages/lc3_env_env_pkg" $env(UVMF_VIP_LIBRARY_HOME)]
quietly set cmd [format "%s %s/environment_packages/lc3_env_env_pkg/lc3_env_env_pkg.sv" $cmd $env(UVMF_VIP_LIBRARY_HOME)]
eval $cmd
